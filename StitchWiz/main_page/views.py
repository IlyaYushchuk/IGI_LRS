...
def polygon(request):
    ...
    return render(request, "polygon.html", content)

def confidentiality(request):
    content = {
        'title':'Main',
    }
    return render(request, "confidentiality.html", content)
...
def staff(request):
    ...
    staff = list(User.objects.filter(is_staff=True).values('first_name', 'last_name', 'position', 'date_joined', 'email', 'phone', 'image'))
